...
//    };
    namespace encryption{
        inline void V1(const char channel,const char* passwd,const char* inpath,const char* outputpath){
    ...
    namespace decrypt{
        inline void V1(const int channel,const char* passwd,const char* inpath,const char* outputpath){
            cv::Mat image = cv::imread(inpath);
            if(image.empty()){
                printf("imgsha:Can't get image for path.\n");
                exit(2);
